...
DATA_PATH = Path('./sleep_intermediate_datafiles/')
DATASET_FNAME = sys.argv[1] if len(sys.argv) > 1 else 'sleep_chapter3_result_outliers.csv'
RESULT_FNAME = sys.argv[2] if len(sys.argv) > 2 else 'sleep_chapter3_result_final.csv'
ORIG_DATASET_FNAME = sys.argv[3] if len(sys.argv) > 3 else 'sleep_chapter2_result.csv'
...
milliseconds_per_instance = (0.644682862209302 - 0.503153066860465)/1000
...
imputed_interpolation_dataset = MisVal.impute_interpolate(copy.deepcopy(dataset), 'acc_phone_y')
imputed_interpolation_dataset = MisVal.impute_interpolate(copy.deepcopy(dataset), 'acc_phone_z')
imputed_interpolation_dataset = MisVal.impute_interpolate(copy.deepcopy(dataset), 'hr_watch_rate')
imputed_interpolation_dataset = MisVal.impute_interpolate(copy.deepcopy(dataset), 'count_phone_steps')
...
periodic_measurements = ['acc_phone_x', 'acc_phone_y', 'acc_phone_z', 'hr_watch_rate', 'count_phone_steps']
...
selected_predictor_cols = [c for c in dataset.columns if (not ('label' in c)) and (not (c == 'count_phone_steps'))]
...
# fig 7
DataViz.plot_dataset(dataset, ['acc_', 'hr_watch_rate', 'pca_', 'label'],
